...
func (laptopClient *LaptopClient) UploadImage(imageId string, imagePath string) {
...
func (laptopClient *LaptopClient) RateLaptop(laptopIDs []string, scores []float64) error {
	...
	go func() {
		for {
			...
			if err != nil {
				waiteReply <- fmt.Errorf("cannot receive stream response: %v", err)
				return
			}
